...
const (
	passwordHash = "passwordHashNow"
)

func HashEncode(input string) (string, error) {
	md5 := md5.New()
	sha_256 := sha256.New()
	sha_512 := sha512.New()
	...
	hmac512 := hmac.New(sha512.New, []byte(passwordHash))
	...
	sencode := base64.StdEncoding.EncodeToString(hmac512.Sum(nil))
	var err error
	if len(strings.TrimSpace(sencode)) == 0 {
		err = errors.New("Erro ao escrever o hash")
	}
